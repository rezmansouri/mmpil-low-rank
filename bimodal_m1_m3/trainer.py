...
def main():
    ...
    if len(sys.argv) != 8:
        raise ValueError(
            'usage: python trainer.py {partitions root} {training partitions} {validation partitions} {# epochs} {output path} {properties encoder path} {ch encoder path}\nexample: python main.py /data 135 2 1000 ./output ./properties.pt ./ch.pt')
    root, training_partitions, validation_partitions, n_epochs_str, output_path, properties_enc_path, ch_enc_path = sys.argv[
        1:]
    ...
    X_train_properties, X_train_ch, _, y_train = util.get_raw_data(
        training_paths)
    ...
    train_dataset = util.CustomDataset(
        [X_train_properties, X_train_ch], y_train)
    ...
    inference_model = model.InferenceModel(
        properties_enc, ch_enc, 128, device).to(device)

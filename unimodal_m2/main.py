    ...
    for path in paths:
        for h5_name in tqdm(os.listdir(path), leave=False):
            with h5py.File(os.path.join(path, h5_name), 'r') as h5:
                X.append(pad_and_resize(h5.get('pil')[:]))
                class_ = h5.get('class')[()]
...
def main():
    ...
    torch.save(best_model_state_dict, os.path.join(
        '/home/mmml_pil/out/prediction/unimodal/pil', f'best_{best_model_epoch}.pth'))

    torch.save(model.state_dict(), os.path.join(
        '/home/mmml_pil/out/prediction/unimodal/pil', f'last_{n_epochs}.pth'))
    ...
    df.to_csv('/home/mmml_pil/out/prediction/unimodal/pil/stats.csv')

...
def main():
    ...
    if len(sys.argv) != 8:
        raise ValueError(
            'usage: python trainer.py {partitions root} {training partitions} {validation partitions} {# epochs} {output path} {pil encoder path} {convexhull encoder path}\nexample: python main.py /data 135 2 1000 ./output ./pil.pt ./ch.pt')
    root, training_partitions, validation_partitions, n_epochs_str, output_path, pil_enc_path, ch_enc_path = sys.argv[
        1:]
    ...
    inference_model = model.InferenceModel(
        pil_enc, ch_enc, 128, device).to(device)

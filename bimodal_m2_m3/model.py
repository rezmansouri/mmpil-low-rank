...
class InferenceModel(nn.Module):
    def __init__(self, pil_encoder, ch_encoder, fusion_output_dim, device):
        super(InferenceModel, self).__init__()
        self.ch_encoder = ch_encoder
        self.pil_encoder = pil_encoder
        self._freeze()
        self.fusion = LowRankFusion(
            rank=4, output_dim=fusion_output_dim, m1_h=256, m2_h=256, device=device)
    ...
    def forward(self, modalities):
        ...
        fused = self.fusion([pil_encoding, ch_encoding])

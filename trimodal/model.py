...
class InferenceModel(nn.Module):
    def __init__(self, properties_encoder, pil_encoder, ch_encoder, fusion_output_dim, device):
        ...
        self.fusion = LowRankFusion(
            rank=8192, output_dim=fusion_output_dim, m1_h=128, m2_h=256, m3_h=256, device=device)
    ...
    def forward(self, modalities):
        properties, pil, ch = modalities
        properties_encoding = self.properties_encoder(properties)[-1]
        pil_encoding = self.pil_encoder(pil)
        ch_encoding = self.ch_encoder(ch)
        fused = self.fusion([properties_encoding, pil_encoding, ch_encoding])
    ...
    def _freeze(self):
        ...
        for param in self.pil_encoder.parameters():
            param.requires_grad = False
        for param in self.ch_encoder.parameters():
            param.requires_grad = False


class LowRankFusion(nn.Module):
    def __init__(self, rank, output_dim, m1_h, m2_h, m3_h, device):
        ...
        self.m3_factor = Parameter(torch.Tensor(
            self.rank, self.m3_h + 1, self.output_dim))
    ...
    def forward(self, modalities):
        ...
        _m2 = torch.cat((m2, torch.autograd.Variable(torch.ones((batch_size, 1)).type(
            torch.float32).to(self.device), requires_grad=False)), dim=1)
        _m3 = torch.cat((m3, torch.autograd.Variable(torch.ones((batch_size, 1)).type(
            torch.float32).to(self.device), requires_grad=False)), dim=1)
        fusion_m1 = torch.matmul(_m1, self.m1_factor)
        fusion_m2 = torch.matmul(_m2, self.m2_factor)
        fusion_m3 = torch.matmul(_m3, self.m3_factor)
        fusion_zy = fusion_m1 * fusion_m2 * fusion_m3

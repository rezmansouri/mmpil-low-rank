...
def collate(batch):
    propertiess, pils, chs, ys = [], [], [], []
    for sample in batch:
        properties, pil, ch, y = sample
    ...
    return (torch.tensor(propertiess, dtype=torch.float32).to(device),
            torch.tensor(pils, dtype=torch.float32).to(device), torch.tensor(chs, dtype=torch.float32).to(device)), torch.tensor(ys, dtype=torch.float32).to(device)


class CustomDataset(torch.utils.data.Dataset):
    def __init__(self, modalities, labels):
        self.properties, self.pil, self.ch = modalities
    ...
    def __getitem__(self, index):
        ...
        return properties, pil, ch, label
